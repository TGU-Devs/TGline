...
```
Request:  { token: "xxx", password: "NewPass123", password_confirmation: "NewPass123" }
Response:
  成功 → { user: { id, email, display_name, role } } + JWT Cookie設定
  失敗 → { errors: ["トークンが無効または期限切れです"] } 422
```

- `User.reset_password_by_token` でDeviseのトークン検証 + パスワード更新
...
- パスワードバリデーションロジックは既存の change-password と共通化できるならユーティリティとして切り出す
...
2. **メールベースの制限**: 同一メールアドレスへのリセット要求を3回/1時間
